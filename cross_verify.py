...
def main():
    ...
    parser.add_argument('--first_annotator', type=str, default='chennai_anotation_XukeHu')
    parser.add_argument('--second_annotator', type=str, default='chennai_anotation_TobiasElssner')

    args = parser.parse_args()
    print ('data: '+str(args.data))
    if args.mode==1:
        print ('You are under annotation mode.')
    # if args.mode==2:
    ...
    annotation_file = base_dir+args.data+'_anotation_gold.json'
    annotation_file1 = base_dir+args.first_annotator+'.json'
    annotation_file2 = base_dir+args.second_annotator+'.json'
    io = open(annotation_file1,"r")
    place_data1 = json.load(io)
    io = open(annotation_file2,"r")
    place_data2 = json.load(io)
    max_place_count = 100
    io = open(file,"r")
    ...
    if args.mode == 1:
        existing_keys = []
        while 1:
            print(colored(0, 100,  255,'Start from last check (1). Start from first unchecked tweet (0). Start from a specific tweet (Tweet ID)'))
            result_index = input()
            if result_index == '1':
    ...
    while (1):
        ...
        while key_index < len(keys):
            ...
            while i < max_place_count:
                ...
                #     i+=1
                #     continue
                if args.mode and (key in place_data1 and key in place_data2 and 'T'+str(i) in place_data2[key] \
                    and 'T'+str(i) in place_data1[key]):
                    place1= place_data1[key]['T'+str(i)]
                    place2 = place_data2[key]['T'+str(i)]
                    dis = hs.haversine((float(place1['lat']),float(place1['lon'])), \
                                (float(place2['lat']),float(place2['lon'])))
                    if dis < distance_thres:
                        perfect_place = place1
                        if place1['osm_id'] == 'none':
                            perfect_place = place2
                        if key not in anno_place_data:
                            anno_place_data[key] = {}
                        anno_place_data[key]['T'+str(i)] = perfect_place
                ...
                if place['type'] == 'ambLoc' or target_place.lower() in ['chennai','puertorico','texas', 'tx', 'houston','la', 'hou', 'khou11', 'housto', 'abc13', 'a','h-town','h town','htown','nws']:
                    ...
                    continue
                while 1:
                    '''annotation of target place name'''
                    existing_annotaion = []
                    ...
                    try:
                        place1= place_data1[key]['T'+str(i)]
                        existing_annotaion.append((place1['osm_id'],place1['type'],place1['class'], place1['lat'],place1['lon'],place1['place_id'], 'first annotator'))
                    except:
                        cc = 0
                    try:
                        place2 = place_data2[key]['T'+str(i)]
                        existing_annotaion.append((place2['osm_id'],place2['type'],place2['class'], place2['lat'], place2['lon'], place2['place_id'],'second annotator'))
                    except:
                        cc = 0
                            # m+=1
                    ...
                    for t, exist in enumerate(existing_annotaion):
                        print(colored(0, 100,  255,'annotation of '+target_place+' by '+ exist[-1]))
                    ...
                    #255,255,0
                    #0, 100,  255, 
                    print(colored(255,255,0, 'Input the index  / Input OSM ID / Input a new name / Input lat and lon coordiantes/ Skip:s') )
                    result_index = input()
                    bool_osm_id_way = 0
                    while 1:
